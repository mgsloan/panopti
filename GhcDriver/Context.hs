module GhcDriver.Context (
  ...
  support_String, support_show) where
...
-- additional digit
newPhantomModule :: Ghc PhantomModule
newPhantomModule =
...
allModulesInContext :: Ghc ([ModuleName], [ModuleName])
allModulesInContext =
    do (l, i) <- runGhc Compat.getContext
       return (map showModuleName l, map showModuleName i)

addPhantomModule :: (ModuleName -> ModuleText)
                 -> Ghc PhantomModule
...
removePhantomModule :: PhantomModule -> Ghc ()
       ...
       safeToRemove <-
           if isLoaded
             then do -- take it out of scope
                     ...
                     --
                     let isNotPhantom = isPhantomModule . showModuleName >=>
                                          return . not
...
modNameFromSummary =  showModuleName . GHC.ms_mod

getLoadedModSummaries :: Ghc [GHC.ModSummary]
...
setTopLevelModules :: [ModuleName] -> Ghc ()
...
setImportsQ :: [(ModuleName, Maybe String)] -> Ghc ()
setImportsQ ms =
    do let qualOrNot = \(a,mb) -> maybe (Right a) (Left . (,) a) mb
           (quals,unquals) = partitionEither $ map qualOrNot ms
       --
